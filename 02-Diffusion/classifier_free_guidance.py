## this code is an adtapted version originally created by the one and only lucidrains
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        num_samples = 25,
        results_folder = './Checkpoint-diffusion', ##to define where we launch this in order to set this path
        amp = False,
        fp16 = False,
        split_batches = True,
        ...
        # adding transformation for the dataset
        # augment_horizontal_flip = False

        # maybe_convert_fn = partial(convert_image_to_fn, convert_image_to) if exists(convert_image_to) else nn.Identity()

        self.folder = folder
        def get_data(image_size = self.image_size, folder = self.folder, batch_size = self.batch_size):
        ...
        # changeing the dataset to be Imagefolder
        dl = get_data()
        ...
        with tqdm(initial = self.step, total = self.train_num_steps, disable = not accelerator.is_main_process) as pbar:
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    row_data = next(self.dl)
                    data , labels = row_data
                    data = data.to(device)
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and self.step % self.save_and_sample_every == 0:
                        ...
                        utils.save_image(all_images_list, str(self.results_folder / f'sample-{milestone}.png'))
                        self.save(milestone)
...
if __name__ == '__main__':
    ...
    fp16 = False #enable mixed precision to save some space
    )
